<template>
  <div>
    ...
    <mdb-col class="pt-2">
      <mdb-btn color="primary" class="ml-0" @click.native="addVideo = true">
        Add
      </mdb-btn>
      <mdb-btn color="secondary" @click.native="selectVideo = true">
        Select
      </mdb-btn>
    </mdb-col>
    <helpers-add-video :upload-video.sync="addVideo" :video.sync="vid" />
    <helpers-select-video :select.sync="selectVideo" :item.sync="vid" :items.sync="videoList" />
  </div>
</template>
