<template>
  <div>
    ...
    <mdb-col class="pt-2">
      <mdb-btn color="primary" class="ml-0" @click.native="addImage = true">
        {{ 'New ' + type }}
      </mdb-btn>
      <mdb-btn color="secondary" @click.native="selectImage = true">
        {{ 'Select ' + type }}
      </mdb-btn>
    </mdb-col>
    <helpers-add-image :upload-image.sync="addImage" :image.sync="img" />
    <helpers-select-item :select.sync="selectImage" :item.sync="img" :items.sync="imageList" />
  </div>
</template>
